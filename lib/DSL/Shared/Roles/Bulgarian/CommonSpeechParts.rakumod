...
role DSL::Shared::Roles::Bulgarian::CommonSpeechParts {
    ...
    token a-determiner:sym<Bulgarian>  { '' }
    ...
    token automatic:sym<Bulgarian> { :i 'автоматичен' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'автоматичен', 2) }> | 'автоматично' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'автоматично', 2) }> }
    ...
    token bottom-noun:sym<Bulgarian>  { 'дъно' | 'най-ниско' }
    ...
    token cluster:sym<Bulgarian>  { <cluster-noun> }
    ...
    token columns:sym<Bulgarian>  { <columns-noun> }
    ...
    token contingency-noun:sym<Bulgarian>  { 'контингентен' | 'кръстосано' 'табулиране' }
    token convert-verb:sym<Bulgarian> { :i 'превърни' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'превърни', 2) }> }
    token count-verb:sym<Bulgarian> { :i 'преброй' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'преброй', 2) }> | 'брой' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'брой', 2) }> }
    token counts-noun:sym<Bulgarian> { :i 'броя' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'броя', 2) }> | 'броеве' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'броеве', 2) }> }
    token create:sym<Bulgarian> { :i 'създай' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'създай', 2) }> | 'направи' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'направи', 2) }> }
    token create-directive:sym<Bulgarian>  { <create-verb> | 'направи' }
    ...
    token data-noun:sym<Bulgarian> { :i 'данни' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'данни', 2) }> }
    token dataset:sym<Bulgarian>  { 'дейтасет' | 'дейта' \h+ 'сет' | 'масив' \h+ 'от' \h+ 'данни' | 'даннов' \h+ 'масив' }
    token dataset-noun:sym<Bulgarian>  { 'дейтасет' | 'масив' \h+ 'от' \h+ 'данни' | 'даннов' \h+ 'масив' }
    token datasets-noun:sym<Bulgarian>  { 'дейтасети' | 'масиви' \h+ 'от' \h+ 'данни' | 'даннови' \h+ 'масиви' }
    token default:sym<Bulgarian>  { <default-noun> }
    token default-noun:sym<Bulgarian>  { 'по' 'подразбиране' | 'подразбиращ' \h+ 'се' }
    ...
    token diagram:sym<Bulgarian>  { <diagram-synonyms> }
    token diagram-noun:sym<Bulgarian> { :i 'диаграма' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'диаграма', 2) }> }
    token diagram-synonyms:sym<Bulgarian>  { <diagram-noun> | <plot-noun> | <plots-noun> | <graph-noun> | <chart-noun> }
    ...
    token display-directive:sym<Bulgarian>  { <display-verb> | 'покажи' | 'ехо' }
    ...
    token drop-verb:sym<Bulgarian> { :i 'изхвърли' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'изхвърли', 2) }> }
    ...
    token element:sym<Bulgarian> { <element-noun> }
    token element-noun:sym<Bulgarian> { :i 'елемент' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'елемент', 2) }> }
    token elements:sym<Bulgarian>  { <elements-noun> }
    ...
    token empty-verb:sym<Bulgarian>  { 'празни' | <empty-noun> }
    ...
    token extract-directive:sym<Bulgarian> { :i 'измъкни' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'измъкни', 2) }> | 'добий' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'добий', 2) }> }
    token filter:sym<Bulgarian>  { <filter-verb> }
    token filter-noun:sym<Bulgarian>  { <filter-verb> }
    ...
    token function:sym<Bulgarian> { :i 'функция' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'функция', 2) }> }
    token functions:sym<Bulgarian> { :i 'функции' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'функции', 2) }> }
    token generate-directive:sym<Bulgarian>  { <generate-verb> | <create-verb> | 'направи' }
    token generate-verb:sym<Bulgarian> { :i 'генерирай' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'генерирай', 2) }> }
    token get-verb:sym<Bulgarian>  { 'вземи' | 'добий' | <take-verb> }
    token graph-noun:sym<Bulgarian> { :i 'граф' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'граф', 2) }> }
    token head-adjective:sym<Bulgarian>  { <head-noun> }
    ...
    token higher-adjective:sym<Bulgarian>  { 'по-висок' }
    ...
    token identifier-noun:sym<Bulgarian> { :i 'идентификатор' | ([\w]+) <?{ $0.Str ne 'ИДЕНТИФИКАТОР' and is-bg-fuzzy-match($0.Str, 'идентификатор', 2) }> }
    token in-preposition:sym<Bulgarian> { :i 'в' }
    ...
    token largest-adjective:sym<Bulgarian>  { 'най-голям' }
    token last-adjective:sym<Bulgarian> { :i 'последен' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'последен', 2) }> }
    ...
    token lower-adjective:sym<Bulgarian>  { 'по-долен' | 'долен' }
    ...
    token maximum:sym<Bulgarian> { :i 'макс' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'макс', 2) }> | 'максимум' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'максимум', 2) }> }
    ...
    token module-noun:sym<Bulgarian> { :i 'модул' | ([\w]+) <?{ $0.Str ne 'МОДУЛ' and is-bg-fuzzy-match($0.Str, 'модул', 2) }> }
    ...
    token nearest-adjective:sym<Bulgarian>  { 'най-близък' | 'най-близки' }
    ...
    token pipeline-noun:sym<Bulgarian>  { 'поточна' 'линия' }
    ...
    token represent-directive:sym<Bulgarian>  { <represent> | 'чертай' | 'отрази' }
    ...
    token results:sym<Bulgarian>  { <results-noun> }
    ...
    token rows:sym<Bulgarian>  { <rows-noun> }
    token rows-noun:sym<Bulgarian> { :i 'редове' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'редове', 2) }> }
    token run-verb:sym<Bulgarian> { :i 'run' | ([\w]+) <?{ $0.Str !(elem) <runs running> and is-bg-fuzzy-match($0.Str, 'run', 1) }> | 'runs' | ([\w]+) <?{ $0.Str !(elem) <run running> and is-bg-fuzzy-match($0.Str, 'runs', 2) }> }
    token running-verb:sym<Bulgarian> { :i 'running' | ([\w]+) <?{ $0.Str !(elem) <runs run> and is-bg-fuzzy-match($0.Str, 'running', 2) }> }
    token separate-verb:sym<Bulgarian> { :i 'раздели' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'раздели', 2) }> }
    token series-noun:sym<Bulgarian> { :i 'серии' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'серии', 2) }> }
    ...
    token simulate-directive:sym<Bulgarian>  { <simulate> }
    ...
    token smallest:sym<Bulgarian>  { 'най-малък' | 'най-малки' }
    token smallest-adjective:sym<Bulgarian>  { 'най-малък' | 'най-малки'  }
    ...
    token statistical:sym<Bulgarian>  { <statistical-adjective> }
    ...
    token summaries:sym<Bulgarian>  { <summaries-noun> }
    ...
    token summary:sym<Bulgarian>  { <summary-noun> }
    ...
    token target-noun:sym<Bulgarian> { :i 'цел' | ([\w]+) <?{ $0.Str ne 'ЦЕЛ' and is-bg-fuzzy-match($0.Str, 'цел', 1) }> }
    ...
    token timeline-noun:sym<Bulgarian>  { 'времева линия' }
    token times-noun:sym<Bulgarian> { :i 'времена' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'времена', 2) }> }
    token to-preposition:sym<Bulgarian> { :i 'за' | 'към' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'към', 1) }> }
    ...
    token versus-preposition:sym<Bulgarian>  { 'срещу' | 'ср.' | 'против' }
    ...
    token weight:sym<Bulgarian>  { <weight-noun> }
    token weights-noun:sym<Bulgarian> { :i 'тегла' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'тегла', 2) }> }
    token weights:sym<Bulgarian>  { <weights-noun> }

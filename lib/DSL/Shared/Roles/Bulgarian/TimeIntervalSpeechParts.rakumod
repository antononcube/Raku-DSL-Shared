...
role DSL::Shared::Roles::Bulgarian::TimeIntervalSpeechParts {
  ...
  token day-time-spec-word:sym<Bulgarian> { :i 'ден' | ([\w]+) <?{ $0.Str !(elem) <дни дек фев пон един една едно едни десен десни сеп чет днес> and is-fuzzy-match($0.Str, 'ден', 1) }> }
  ...
  token last-time-spec-word:sym<Bulgarian> { :i 'последен' | ([\w]+) <?{ $0.Str !(elem) <последна последно последни> and is-fuzzy-match($0.Str, 'последен', 2) }> | 'последна' | ([\w]+) <?{ $0.Str !(elem) <последен последно последни> and is-fuzzy-match($0.Str, 'последна', 2) }> | 'последно' | ([\w]+) <?{ $0.Str !(elem) <последен последна последни> and is-fuzzy-match($0.Str, 'последно', 2) }> | 'последни' | ([\w]+) <?{ $0.Str !(elem) <последен последна последно> and is-fuzzy-match($0.Str, 'последни', 2) }> }
  ...
  token month-time-spec-word:sym<Bulgarian> { :i 'месец' | ([\w]+) <?{ $0.Str !(elem) <месеци месеца десен> and is-fuzzy-match($0.Str, 'месец', 2) }> }
  token months-time-spec-word:sym<Bulgarian> { :i 'месеци' | ([\w]+) <?{ $0.Str !(elem) <месец месеца> and is-fuzzy-match($0.Str, 'месеци', 2) }> | 'месеца' | ([\w]+) <?{ $0.Str !(elem) <месец месеци> and is-fuzzy-match($0.Str, 'месеца', 2) }> }
  ...
  token right-time-spec-word:sym<Bulgarian> { :i 'вдясно' | ([\w]+) <?{ $0.Str !(elem) <дясна дясно> and is-fuzzy-match($0.Str, 'вдясно', 2) }> | 'десен' | ([\w]+) <?{ $0.Str !(elem) <ден месец десни> and is-fuzzy-match($0.Str, 'десен', 2) }> | 'дясна' | ([\w]+) <?{ $0.Str !(elem) <вдясно дясно десни> and is-fuzzy-match($0.Str, 'дясна', 2) }> | 'дясно' | ([\w]+) <?{ $0.Str !(elem) <вдясно дясна десни> and is-fuzzy-match($0.Str, 'дясно', 2) }> | 'десни' | ([\w]+) <?{ $0.Str !(elem) <ден дни едни десен дясна дясно> and is-fuzzy-match($0.Str, 'десни', 2) }> }

...
#==============================================================================
#
#   Common Structures Raku actions in Raku (Perl 6)
...
unit module DSL::Shared::Actions::Raku::CommonStructures;

class DSL::Shared::Actions::Raku::CommonStructures
    ...
    method r-range-spec($/) { make 'Range.new(' ~ $<number-value-list>.made.substr(1,*-1) ~ ")"; }
    method wl-range-spec($/) { make 'Range.new(' ~ $<number-value-list>.made.substr(1,*-1) ~ ")"; }
    ...
    method range-spec($/) {
        if $<range-spec-step> {
            make $<range-spec-from>.made ~ ', ' ~ '(' ~ $<range-spec-from>.made ~ ' + ' ~ $<range-spec-to>.made ~ ') ... ' ~ $<range-spec-step>.made;
        } else {
            make $<range-spec-from>.made ~ ' ... ' ~ $<range-spec-to>.made;
        }
    }
    ...
    # Regex
    method regex-pattern-spec($/) { make 'rx/' ~ $<regex-pattern>.made ~ '/'; }

    # Over-writing the trivial
    # Trivial
    method trivial-parameter($/) { make $/.values[0].made; }
    method trivial-parameter-none($/) { make 'Nil'; }
    method trivial-parameter-empty($/) { make '()'; }
    method trivial-parameter-automatic($/) { make 'Whatever'; }
